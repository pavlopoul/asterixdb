    ...
    private Boolean constructAssign(InnerJoinOperator join, LogicalVariable lvl, LogicalVariable lvr) {
        ...
        if (join.getInputs().get(0).getValue().getOperatorTag() == LogicalOperatorTag.SELECT
            ...
            if ((join.getInputs().get(0).getValue().getCardinality() < 50
                    && dsl.getDataset().getDatasetType() == DatasetType.READER)
                    || (join.getInputs().get(1).getValue().getCardinality() < 50
                            && dsr.getDataset().getDatasetType() == DatasetType.READER)) {
        ...
        if (leftField != null && rightField != null) {
            ...
            if (assignl != null) {
                assignl.setCardinality(context.getCardinalityEstimator().getTableCardinality(
                        context.getMetadataProvider(), datasourcel.getDataset().getDataverseName().getCanonicalForm(),
                        datasourcel.getDataset().getDatasetName(), leftField));
            } else {
                op.getInputs().get(0).getValue().setCardinality(context.getCardinalityEstimator().getTableCardinality(
                        context.getMetadataProvider(), datasourcel.getDataset().getDataverseName().getCanonicalForm(),
                        datasourcel.getDataset().getDatasetName(), leftField));
            }
            if (assignr != null) {
                ...
                op.getInputs().get(1).getValue().setCardinality(context.getCardinalityEstimator().getTableCardinality(
                        context.getMetadataProvider(), datasourcer.getDataset().getDataverseName().getCanonicalForm(),
                        datasourcer.getDataset().getDatasetName(), rightField));
            }
            System.out.println(
            ...
            if (!multipleConditions) {
                return context.getCardinalityEstimator().getJoinCardinality(context.getMetadataProvider(),
                        datasourcel.getDataset().getDataverseName().getCanonicalForm(),
                        datasourcel.getDataset().getDatasetName(), leftField,
                        datasourcer.getDataset().getDataverseName().getCanonicalForm(),
                        datasourcer.getDataset().getDatasetName(), rightField);
            } else {
                        ...
                        if (leftField != null) {
                            ...
                            if (lowKeyValue != null && highKeyValue != null) {
                                tempCardinality = context.getCardinalityEstimator().getRangeCardinality(
                                        ...
                                        ds.getDataset().getDataverseName().getCanonicalForm(),
